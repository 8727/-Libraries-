...
void Sim80x_SendString(char *str){
  HAL_UART_Receive_IT(&SIM80X_USART,&Sim80x.UsartRxTemp,1);
  #if(SIM80X_DMA_TRANSMIT)
    while(SIM80X_USART.hdmatx->State != HAL_DMA_STATE_READY) osDelay(10);
    HAL_UART_Transmit_DMA(&SIM80X_USART,(uint8_t*)str,strlen(str));
    while(SIM80X_USART.hdmatx->State != HAL_DMA_STATE_READY) osDelay(10);
  #else
    HAL_UART_Transmit(&SIM80X_USART,(uint8_t*)str,strlen(str),100);
    osDelay(10);
  #endif
}
...
void Sim80x_SendRaw(uint8_t *Data,uint16_t len){
  ...
  #if(SIM80X_DMA_TRANSMIT)
    while(SIM80X_USART.hdmatx->State != HAL_DMA_STATE_READY) osDelay(10);
    HAL_UART_Transmit_DMA(&SIM80X_USART,Data,len);
    while(SIM80X_USART.hdmatx->State != HAL_DMA_STATE_READY) osDelay(10);
  #else
    HAL_UART_Transmit(&SIM80X_USART,Data,len,100);
    osDelay(10);
  #endif
}
...
void Sim80x_RxCallBack(void){
  if((Sim80x.Status.DataTransferMode==0)&&(Sim80x.UsartRxTemp!=0)){
    ...
    if(Sim80x.UsartRxIndex < (_SIM80X_BUFFER_SIZE-1)) Sim80x.UsartRxIndex++;
  }
...
uint8_t Sim80x_SendAtCommand(char *AtCommand,int32_t  MaxWaiting_ms,uint8_t HowMuchAnswers,...){
  while(Sim80x.Status.Busy == 1){ osDelay(100);
  }
...
void Sim80x_BufferProcess(void){
  ...
  //##################################################
  str1 = strstr(strStart,"\r\nCall Ready\r\n");
  if(str1!=NULL) Sim80x.Status.CallReady=1;
  //##################################################
  str1 = strstr(strStart,"\r\nSMS Ready\r\n");
  if(str1!=NULL) Sim80x.Status.SmsReady=1;  
  //##################################################
  str1 = strstr(strStart,"\r\n+COLP:");
  if(str1!=NULL){
  ...
  if(str1!=NULL){
    ...
    if(tmp_int32_t == 3) Sim80x.Bluetooth.ConnectedID = 255;
  }
  //##################################################
  str1 = strstr(strStart,"\r\n+BTPAIR:");
  if(str1!=NULL){
  ...
  memset(Sim80x.UsartRxBuffer,0, SIM80X_BUFFER_SIZE);
  Sim80x.Status.Busy=0;
}
...
void StartSim80xTask(void const * argument){ 
  ...
  while(1){
    ...
    if(Sim80x.Gsm.MsgUsed > 0){
      ...
      UnreadMsgCounter++;
      if(UnreadMsgCounter==150) UnreadMsgCounter=0;
    }
    //###########################################
